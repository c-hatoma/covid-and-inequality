...
```{r}
km.covid.eviction.clust <- covid.eviction.clust[,-c(18, 22:25)]
km.covid <- km.covid.eviction.clust %>%
...
tot <- NULL
for(i in 1:20){
 km <- kmeans(km.covid %>% select(-State, -levels),
              i)
...
Just from looking at the second and third graphs, we see that there are overlaps, while there is almost none in the first graph when k = 2. This makes sense, because there are more than just the variables from the x- and y-axes that we are analyzing the data from. 

The fact that there is an overlap even when k = 2 suggests that it is almost impossible to cluster perfectly. However, although we want (almost) a perfect separation as shown in the first graph, a k-value as small as 2 is skeptical -- it is skeptical to cluster many observations into just two groups when there are multiple variables to consider. Let's dig a little more deeply. 
...
As we see with total within-variation, the case in which k = 2 has the highest variation, almost as twice as when k = 6 and almost the sum of when k = 4 and k = 6. It is by no surprise that the higher the k-value, the less variation of the dataset. And with that observation, we would initially assume that the higher k-value would lead to more accurate clustering. However, the results from the tables show otherwise. When we look at "low case" (the third column), there are "low case" situations in all four k segments, but there are fewer than there are in k = 6 segments. Not only does this prove our assumption to be false, but it also serves as an example of overfitting. Additionally, this observation shows that it is hard to determine the optimal value of k. It really boils down to what we prioritize --  for example, are we focusing on having the least variation? or the accuracy?


## Overall analysis of efficiency of unsupervised learning techniques
Hierarhical: geared more towards lookinga at the level of dissimiarility of accuracies and allows us to observe which states are more similar and different from one another. does somewhat answer to our research question that focuses on relevance of socioeconomic factors like eviction rate, poverty rate, income, etc. with covid case and death rates, but is only restricted to our assumption (like the effect of geographical proximity on clustering) and cannot confirm solely from hierarchical clustering. also restricted to looking at one condition at one time (e.g. covid case rate < us level)

k-means: hard to determine the optimal k; time consuming to create a "perfect" graph that shows clear clusters, esp when k-means visualization shows organized clusters in two dimensions only. each k has its drawbacks, regardless of how high or low it is. overlaps exist even in k as low as 2, so we should take that as given. 
but allows us to look at the dataset more holistically, that is with all four cases with covid cases and deaths
